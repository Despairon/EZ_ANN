...
namespace EZ_ANN_4_Letter_Recognition
{
    ...
    public partial class mainForm : Form
    {
        public mainForm()
        {
            ...
            result[0] = "0 | 0 = 1 on " + outputs[0].ToString() + "\n";//string.Format("{0:#.###}",outputs[0]*100) + "%\n";

            outputs = ann.recognize(new double[] { 0, 1 });

            result[1] = "0 | 1 = 1 on " + outputs[0].ToString() + "\n";//string.Format("{0:#.###}", outputs[0]*100) + "%\n";

            outputs = ann.recognize(new double[] { 1, 0 });

            result[2] = "1 | 0 = 1 on " + outputs[0].ToString() + "\n";//string.Format("{0:#.###}", outputs[0] * 100) + "%\n";

            outputs = ann.recognize(new double[] { 1, 1 });

            result[3] = "1 | 1 = 1 on " + outputs[0].ToString() + "\n";//string.Format("{0:#.###}", outputs[0] * 100) + "%\n";
